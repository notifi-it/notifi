...
{"ok":true,"sent":1,"results":[{"key":"nk_abcd","ok":true,"warnings":["Title shortened to ${TITLE_MAX} characters."]}],"warnings":["Title shortened to ${TITLE_MAX} characters."]}`;
...
const RESPONSES: Group[] = [
  {
    ...
    file: 'accepted',
    lang: 'http',
    code: responseBody(202, 'Accepted', '{"ok":true,"sent":1,"results":[{"key":"nk_abcd","ok":true}]}'),
  },
  ...errors
...
${terminalGroup('r-', 'Responses', RESPONSES)}
    <p>
      <code>sent</code> counts the devices that received the notification and
      <code>results</code> has one entry per key, named by the prefix the Keys tab shows.
      A <code>warnings</code> array is present only when the
      notification was delivered differently from what was asked: a cropped title or body,
      or <code>is_critical</code> on a key without urgent alerts. The status is still
...
${pre(WARNINGS_RESPONSE, 'http')}
    ...
    <p>
      ...
      and a Mac. Each device is delivered to separately, with its own rate limit and
      its own entry in <code>results</code>. The status is <code>202</code> if at least
      one device received the notification, otherwise the first error's. Two keys on the
      same platform answer <code>400 invalid_request</code>.
    </p>
