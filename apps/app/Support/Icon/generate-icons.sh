...
DEV="$ASSETS/AppIconDev.appiconset"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
python3 - "$TMP/two-tone.svg" "$TMP/two-tone-dev.svg" <<'PY'
import re, sys
src = open("notifi-logo.svg").read()
BRAND = "rgb(73.699951%, 12.89978%, 13.299561%)"
for path, body in ((sys.argv[1], "rgb(92.9%, 92.9%, 92.9%)"), (sys.argv[2], "rgb(11%, 11%, 12%)")):
  out, cursor = [], 0
  for i, m in enumerate(re.finditer(r"<path[^>]*>", src)):
    colour = "rgb(85.9%, 29%, 29.4%)" if i == 1 else body
...
for v in two-tone two-tone-dev; do
  rsvg-convert -w 2400 -h 2400 "$TMP/$v.svg" -o "$TMP/$v-raw.png"
  magick "$TMP/$v-raw.png" -trim +repage "$TMP/$v-logo.png"
done
cp "$TMP/two-tone-logo.png" "$TMP/logo.png"
magick "$TMP/bg.png" -channel RGB -negate +channel "$TMP/bg-dev.png"

glyph() {
  magick "$TMP/${3:-two-tone}-logo.png" -resize "x$1" \
...
glyph 600 "$TMP/g-ios-dev.png" two-tone-dev
magick "$TMP/bg-dev.png" "$TMP/g-ios-dev.png" -gravity center -composite "$DEV/icon-1024.png"
...
mac() {
  magick "$TMP/$1" "$TMP/plate-mask.png" -alpha off -compose CopyOpacity -composite "$TMP/plate.png"
  magick "$TMP/plate.png" -fill none -stroke "$2" -strokewidth 3 \
  ...
  glyph 490 "$TMP/g-mac.png" "$3"
  magick "$TMP/plate-sh.png" "$TMP/g-mac.png" -gravity center -geometry +0-10 -composite "$TMP/mac.png"
  for sz in 16 32 64 128 256 512 1024; do
    magick "$TMP/mac.png" -resize "${sz}x${sz}" "$4/mac-$sz.png"
  done
}

mac bg.png "rgba(255,255,255,0.10)" two-tone "$OUT"
mac bg-dev.png "rgba(0,0,0,0.10)" two-tone-dev "$DEV"

echo "Wrote $(cd "$OUT" && pwd)"
echo "Wrote $(cd "$DEV" && pwd)"
